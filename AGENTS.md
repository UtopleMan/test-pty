...
Don't look in the non-ai/ folder. Both opinionated shipped approval templates
(`read-only-review`, `csharp-dotnet-development`) now enforce that with `Deny` rules on
`FileRead:non-ai/**` and `FileWrite:non-ai/**`, so the tools and the sandboxed shell refuse it once
either template is applied. The instruction still stands on its own: the `baseline` template carries
no denies, and a native escalation (`sh script.sh`) reads whatever it likes.

Approvals are one rule model — `kind : target : state` in `~/.duetui/rules.json`, per workspace —
covering tools, shell command lines, file reads and writes, network hosts and skill loads. There is
no separate grants store, no capability broker, and no yolo mode: yolo is a template. See the
Security architecture section of README.md before changing any of it.
